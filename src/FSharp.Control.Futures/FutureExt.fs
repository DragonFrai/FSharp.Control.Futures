...
module Future =
    ...
    let inline catch (source: Future<'a>) : Future<Result<'a, exn>> =
        Future.create (fun () -> Computation.catch (Future.runRaw source))
    ...
    let runSync (fut: Future<'a>) : 'a =
        ...
        Computation.runSync (Future.runRaw fut)
    ...
    module Seq =
        ...
        let inline iterAsync (source: 'a seq) (body: 'a -> Future<unit>) =
            Future.create (fun () -> Computation.Seq.iterAsync source (body >> Future.runRaw))
